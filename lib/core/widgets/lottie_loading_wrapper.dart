...
/// Full-screen Lottie map placeholder. Shown on Home until map is ready.
class LottieMapLoading extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor = isDark ? AppConstants.BLACK : AppConstants.GRAY_LIGHT;
    ...
    return Container(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            Lottie.asset(lottieAsset, width: 80, height: 80, fit: BoxFit.contain, repeat: true),
            if (showTitle) ...[
              const SizedBox(height: _kSpaceBetweenItems),
              Text(title, style: Theme.of(context).textTheme.titleLarge?.copyWith(color: textColor), maxLines: 2, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center),
            ],
            if (showSubtitle) ...[
              const SizedBox(height: _kSpaceBetweenTitleSubtitle),
              Text(subtitle, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: textColor), maxLines: 2, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis),
            ],
          ],
        ),
      ),
